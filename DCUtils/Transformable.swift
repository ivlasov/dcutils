...
internal class AsTransform<Q:TransformableType,W:TransformableType>: Transformable {
    ...
    public func transform(value: T?) -> K? {
        do {
            return try direct(value)
        } catch {
            print("Cannot convert \(String(describing: value)) to \(K.transformableType)")
        }
        return nil
    }
    ...
    public func transform(value: K?) -> T? {
        do {
            return try reverse(value)
        } catch {
            print("Cannot convert \(String(describing: value)) from \(K.transformableType) to \(T.transformableType)")
        }
        return nil
    }
    ...
    init(_ direct: @escaping DirectTransform, _ reverse: @escaping ReverseTransform) rethrows {
        self.direct = direct
...
private enum AsTransformableImplementaion {
    ...
    static func preload() {
        String  .asRegister(type: Bool.self,    direct: { ($0 as NSString?)?.boolValue      }, reverse: { "\($0 ?? false)"})
        String  .asRegister(type: Int.self,     direct: { ($0 as NSString?)?.integerValue   }, reverse: { $0 == nil ? nil : "\($0 ?? 0)"        })
        String  .asRegister(type: Float.self,   direct: { ($0 as NSString?)?.floatValue     }, reverse: { $0 == nil ? nil : "\($0 ?? 0.0)"      })
        String  .asRegister(type: Double.self,  direct: { ($0 as NSString?)?.doubleValue    }, reverse: { $0 == nil ? nil : "\($0 ?? 0.0)"      })
